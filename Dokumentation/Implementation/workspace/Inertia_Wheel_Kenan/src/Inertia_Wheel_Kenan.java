...
public class Inertia_Wheel_Kenan {
	...
	// sleep can only take in long which is bounded, greater than zero
		...
		// For the control
		static long control_t_1=0;
		static long control_t_2 =0;
		...
		while(Button.ESCAPE.isUp()){
			...
			////////////////////////////////////////////////
			  ...
			  control_t_2 = System.currentTimeMillis();
				delta_t = control_t_2 - control_t_1;
				if(SAMPLE_TIME_CONTROL >= delta_t){
					Thread.sleep(   SAMPLE_TIME_CONTROL -  delta_t    );
					u = -1*( (L1 * x1Old) +( L2 * x2Old) +( L3 * x3Old) ) ;
				}else{
				count_control =+1;
				}
				control_t_1 = System.currentTimeMillis() ;  
			  
			LCD.drawString(""+x1Old, 0, 1);
			LCD.drawString(""+x2Old, 0, 2);
			LCD.drawString(""+x3Old, 0, 3);
			LCD.drawString(""+count_theta, 0, 5);
			power = (int) (Ku * u) ;
